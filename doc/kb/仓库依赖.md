...
|---------|---------|------------|---------|------|
| 本地文件系统 | `public/app.js` | File System Access API（`showDirectoryPicker`，`mode:'readwrite'`） | `LightMDKit/public/app.js:1254` | 用户选择目录，读取 Markdown 与相对路径图片 |
| 本地文件系统 | `public/app.js` | `FileSystemFileHandle.getFile()` | `LightMDKit/public/app.js:1117`、`LightMDKit/public/app.js:1202` | 读单文件内容 / 读图片转 ObjectURL |
| 本地文件系统 | `public/app.js` | `FileSystemFileHandle.createWritable()` | `LightMDKit/public/app.js:1564` | 编辑模式与 3s 自动保存写回原文件 |
...
| IndexedDB | `public/app.js` | 原生 `indexedDB`（DB `lightmdkit` / store `drops`） | `LightMDKit/public/app.js:1889-1941` | 跨标签页传递拖放得到的 `FileSystemHandle`（句柄可结构化克隆） |
| localStorage | `public/app.js` | 原生 `localStorage` | `LightMDKit/public/app.js:51-66`、`LightMDKit/public/app.js:72-87` | 记忆界面模式（传统/现代）与侧边栏视图（文件列表/目录） |
| 内存 | `public/app.js` | `Map` + `URL.createObjectURL` | `LightMDKit/public/app.js:107`、`LightMDKit/public/app.js:1164-1169`、`LightMDKit/public/app.js:1203` | 图片 ObjectURL 缓存，切换文件夹时 `revokeObjectURL` 释放 |
...
| Mermaid (`@11`) | `public/index.html` / `public/app.js` | 渲染 ` ```mermaid ` 代码块 | `LightMDKit/public/index.html:110`、`LightMDKit/public/app.js:1159` | 弱（`app.js:1141` 判空后跳过） |
| marked (`@12`，浏览器侧) | `public/index.html` / `public/app.js` | 前端 Markdown 解析（实时预览） | `LightMDKit/public/index.html:111`、`LightMDKit/public/app.js:1121` | 强 |
...
| 3 | CodeMirror 5 内核 | `codemirror@5` | 仅锁主版本 | `LightMDKit/public/index.html:103` |
| 4 | addon: `mode/overlay` | `codemirror@5` | 仅锁主版本 | `LightMDKit/public/index.html:104` |
| 5 | addon: `selection/active-line` | `codemirror@5` | 仅锁主版本 | `LightMDKit/public/index.html:105` |
| 6 | mode: `markdown` | `codemirror@5` | 仅锁主版本 | `LightMDKit/public/index.html:108` |
| 7 | mode: `gfm` | `codemirror@5` | 仅锁主版本 | `LightMDKit/public/index.html:109` |
| 8 | Mermaid | `mermaid@11` | 仅锁主版本 | `LightMDKit/public/index.html:110` |
| 9 | marked | `marked@12` | 仅锁主版本 | `LightMDKit/public/index.html:111` |
...
| 被依赖对象 | 依赖类型 | 调用方式 | 代码位置 | 强度 |
|-----------|---------|---------|---------|------|
| `MdRender`（`public/md-render.js`） | 本地模块（全局变量） | `MdRender.renderMarkdown(text, marked)` | `LightMDKit/public/app.js:1121`、`LightMDKit/public/app.js:1694`、`LightMDKit/public/app.js:1721` | 强 |
| `marked` | CDN 全局变量 | 作为参数传入 `MdRender.renderMarkdown` | `LightMDKit/public/app.js:1121` | 强 |
| `CodeMirror` | CDN 全局变量 | `CodeMirror.fromTextArea()`，`mode: gfm`、`theme: 'github'`、`styleActiveLine` | `LightMDKit/public/app.js:112-135` | 强（初始化失败会回退 textarea，但现代模式整体失效） |
| `mermaid` | CDN 全局变量 | `mermaid.initialize()` + `mermaid.run()` | `LightMDKit/public/app.js:140-146`、`LightMDKit/public/app.js:1159` | 弱（`typeof mermaid === 'undefined'` 时直接 return） |
| `public/index.html` | 宿主文档 | 通过 `document.getElementById` 绑定 20+ 个 DOM 节点 | `LightMDKit/public/app.js:2-28` | 强 |

#### `public/index.html`（页面）依赖

| 被依赖对象 | 依赖类型 | 调用方式 | 代码位置 | 强度 |
|-----------|---------|---------|---------|------|
| `public/style.css` | 本地资源 | `<link rel="stylesheet" href="style.css?v=25">` | `LightMDKit/public/index.html:19` | 强 |
| `public/md-render.js` | 本地资源 | `<script src="md-render.js?v=5">`（UMD 暴露 `window.MdRender`） | `LightMDKit/public/index.html:112` | 强 |
| `public/app.js` | 本地资源 | `<script src="app.js?v=33">` | `LightMDKit/public/index.html:113` | 强 |
| 9 个 CDN 资源 | 外部资源 | `<link>` / `<script>` | `LightMDKit/public/index.html:10,18,89-95` | 强 |

> `style.css` 与 `app.js` 使用 `?v=N` 查询串做缓存失效（`style.css?v=25`、`md-render.js?v=5`、`app.js?v=33`），加上 `index.html:6-8` 的 `Cache-Control` / `Pragma` / `Expires` 三重 no-cache meta —— 这是没有构建工具时的手工缓存策略，**改前端文件后必须同步递增对应的 `?v=`**。
...
| `public/index.html` → `public/app.js` | `<script>` 全局 `window.MdRender` | `LightMDKit/public/index.html:112`、`LightMDKit/public/app.js:1121` |
...
| API | 使用模块 | 用途 | 代码位置 | 强度 |
|-----|---------|------|---------|------|
| `window.showDirectoryPicker()` | `app.js` | 选择本地目录（`mode:'readwrite'`） | `LightMDKit/public/app.js:1248`、`LightMDKit/public/app.js:1254` | **强**（不支持则核心功能不可用） |
| `FileSystemDirectoryHandle.entries()` | `app.js` | 递归遍历目录（深度上限 6、文件数上限 3000） | `LightMDKit/public/app.js:234-258` | 强 |
| `getFileHandle` / `getDirectoryHandle` | `app.js` | 按路径逐级定位文件与子目录（含图片） | `LightMDKit/public/app.js:1099`、`LightMDKit/public/app.js:1197-1199` | 强 |
| `FileSystemFileHandle.getFile()` | `app.js` | 读取文件内容 / 图片 Blob | `LightMDKit/public/app.js:1117`、`LightMDKit/public/app.js:1202`、`LightMDKit/public/app.js:2002` | 强 |
| `FileSystemFileHandle.createWritable()` | `app.js` | 写回原文件（编辑保存 + 3s 自动保存） | `LightMDKit/public/app.js:1558-1566` | 强 |
| `FileSystemHandle.isSameEntry()` | `app.js` | 判断拖入文件是否在当前已加载目录内 | `LightMDKit/public/app.js:1953` | 弱（失败仅降级为「只打开单文件」） |
| `DataTransferItem.getAsFileSystemHandle()` | `app.js` | 拖放时获取文件/目录句柄 | `LightMDKit/public/app.js:1989-1991` | 弱（不可用时回退 `getAsFile()`） |
| `indexedDB` | `app.js` | 跨标签页传递 FileSystemHandle | `LightMDKit/public/app.js:1889-1941` | 弱（不可用时拖放打开降级） |
| `IntersectionObserver` | `app.js` | 滚动时高亮 TOC 当前章节 | `LightMDKit/public/app.js:2117-2125` | 弱（无则目录不高亮，不影响阅读） |
| `localStorage` | `app.js` | 记忆界面模式与侧边栏视图 | `LightMDKit/public/app.js:51-87` | 弱（隐私模式下 try/catch 回退默认值） |
| `URL.createObjectURL` / `revokeObjectURL` | `app.js` | 相对路径图片转 ObjectURL 并缓存 | `LightMDKit/public/app.js:1166`、`LightMDKit/public/app.js:1203` | 弱 |
| `crypto.randomUUID()` | `app.js` | 生成拖放记录的 IndexedDB 主键 | `LightMDKit/public/app.js:1972-1974` | 弱（有 `Date.now()+Math.random()` 回退） |

> **浏览器兼容性硬约束**：`showDirectoryPicker` 目前仅 Chromium 系（Chrome / Edge 86+）实现，**Firefox 与 Safari 不支持**。`app.js:1235` 有特性检测并提示「浏览器不支持文件夹选择，请使用 Chrome 或 Edge」，但此时应用无任何可用功能（因为前端不调用后端 REST API）。`LightMDKit/README.md:45` 已明确要求 Chrome / Edge。
>
...
- 但 `app.js` 里**必须保留 `theme: 'github'`**（`LightMDKit/public/app.js:125`）——它的作用不是加载主题文件，而是让容器带上 `cm-s-github` 类。一旦摘掉，CodeMirror 会回落到 `cm-s-default`，基础主题那批 `.cm-s-default .cm-*` 颜色就会生效，**反而改变现有观感**。
...
- 规避方式：`mode.strikethrough: false`（`LightMDKit/public/app.js:121`，注释见 `app.js:112-116`）；真正的 `~~...~~` 由 `markText` 单独标注（`refreshStrikeMarks`，`LightMDKit/public/app.js:436-936`）。
...
- 规避方式：`patchStrikethrough()` 覆盖 tokenizer，只认 `~~`（`LightMDKit/public/md-render.js:164-188`），`renderMarkdown` 内每次调用前自动打补丁。
...
- 规避方式：`mergeTableCells()` 预处理，把跨行单元格合并回同一行并用 `<br>` 表示换行（`LightMDKit/public/md-render.js:96-156`）。
...
`md-render.js` 是 UMD 模块，同时被 Node（`LightMDKit/server.js:7`）与浏览器（`index.html:112`）加载。修改它时必须保证：

- 不引用任何 DOM / Node 专有 API（当前为纯字符串处理，符合约束）；
- UMD 包装头（`md-render.js:1-7`）不能破坏，否则会同时打断两端；
- Node 侧 `require` 的路径是 `./public/md-render.js` —— **后端反向依赖前端目录**。若重排（如把 `md-render.js` 移出 `public/`），必须同步改 `LightMDKit/server.js:7`、`index.html:112` 与 `pkg.assets` 三处。

### 9. 手工缓存失效策略
无构建工具，前端资源靠 URL 查询串破缓存：`style.css?v=25`、`md-render.js?v=5`、`app.js?v=33`（`LightMDKit/public/index.html:19,112,113`）。
