...
| 存储类型 | 用途 | 版本要求 | 配置位置 |
|-----------|------|---------|---------|
| 文件系统 | Markdown 文件读写 | Node.js 内置 fs 模块 | `LightMDKit/server.js` |
...
|--------|-----|------|
| PORT | 3456 | 服务监听端口 |
| 环境变量 | `LIGHTMDKIT_DAEMON` | 守护进程标识，值为 `1` 时表示守护进程模式 |
...
1. 在 `LightMDKit/package.json` 中添加数据库依赖（如 `mysql2`, `sequelize` 等）
2. 创建数据库配置文件
3. 定义 Model/Schema 文件
4. 创建数据库迁移文件
5. 在 `LightMDKit/server.js` 中初始化数据库连接
...
**参考现有代码**: `LightMDKit/server.js` (第 116-130 行，文件扫描逻辑)

---

> 本文档分析了 LightMDKit 项目的全部数据结构。该项目为轻量级 Markdown 查看器，不使用数据库或中间件，数据完全基于文件系统（File System Access API + Node.js fs 模块）进行读写。
