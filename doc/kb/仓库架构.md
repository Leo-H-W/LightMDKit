...
| **Markdown 解析（浏览器）** | marked 12（CDN） | GFM 解析 | `LightMDKit/public/index.html:111` 以 `<script>` 引入全局 `marked`，运行时以参数注入渲染内核 |
...
| **编辑器** | CodeMirror 5（CDN） | GFM 编辑 + 现代模式实时预览 | `LightMDKit/public/app.js:111-165` 在 `try/catch` 中初始化，失败则降级为 `#editor` 原生 `<textarea>`（`LightMDKit/public/app.js:162-165`） |
| **图表渲染** | Mermaid 11（CDN） | ` ```mermaid ` 代码块渲染 | `LightMDKit/public/app.js:140-146` 惰性 `initialize`；`renderMermaid()`（`LightMDKit/public/app.js:1153-1163`）按需 `mermaid.run`，现代模式下**主动跳过**（`LightMDKit/public/app.js:1156`） |
...
- **依赖注入而非直接引用**：`marked` 由调用方传入（`renderMarkdown(markdown, marked)`，`LightMDKit/public/md-render.js:190`），使同一份代码既能在 Node 用 npm 包、又能在浏览器用 CDN 全局变量。
...
| **渲染发生地** | 浏览器（CDN 版 marked，`LightMDKit/public/app.js:1121`） | Node（npm 版 marked，`LightMDKit/server.js:154`） |
...
- **建议**：至少拆成 4 个 ES Module —— `state.js`（状态与常量）、`scanner.js`（`collectMarkdownFiles` / `flattenByDirectory` / `findEntryByPath`）、`toc.js`（标题解析与目录渲染）、`app.js`（用例编排与 DOM 绑定）。这些模块之间没有循环依赖（`collectMarkdownFiles` 只依赖 `isMarkdownName` 与两个常量），拆分成本低。注意当前 CodeMirror/Mermaid 依赖 CDN 全局变量，拆分前需先确认加载顺序由 `LightMDKit/public/index.html:103-113` 显式保证。
