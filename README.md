...
- **表格编辑** — 现代模式下表格可直接编辑：`Tab` 新增一行、`Enter` 跳到下一行同一格、`Ctrl+Enter` 在单元格内插入换行（单元格随之变成多行，同行其它格子的高度一起撑开、位置不动）、`Backspace` / `Delete` 删的是整个 `<br>` 而不是一个一个字符，而且**只管当前单元格的内容**：顶到格子边界、行首 / 行尾都停住，不会吃掉 `|`、不会删隔壁格子的内容，也不会吞换行把两行并起来（只有确实渲染成表格的行才这么管，选中 `|` 或换行再删仍可手动删结构）
