...
- **表格编辑** — 现代模式下表格可直接编辑：`Tab` 新增一行、`Enter` 跳到下一行同一格、`Ctrl+Enter` 在单元格内插入换行（单元格随之变成多行，同行其它格子的高度一起撑开、位置不动）、`Backspace` / `Delete` 删的是整个 `<br>` 而不是一个一个字符，且在单元格边界会跳过 `|` 接着删隔壁格子（不会把两格并成一格，顶到行首 / 行尾也不吞换行；真要删 `|` 或换行，选中它们再删）
